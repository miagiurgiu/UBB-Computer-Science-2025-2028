...
- binary search instead of linear search (but not visible from the outside) used for search and nrOccurrences -> complexity O(log n)
- sorting criteria
- only init is changed (in the interface; the only change is that init will take a relation as parameter; like giving a function as a parameter; and that function is a 2-parameter function which returns true or false)
...
- unidirectional vs bidirectional iterator
```
vector v
...
- how to implement getCurrent? using a loop starting from head until the value of current element -> not efficient -> see code
...
- the way in which we work with those positions is different, it is similar to the way we work with linked lists
...
- find the node - insert AFTER (like in sll)
```
subalgorithm insertPos(dlla, e,pos):
	...
	if pos = 1 then // case 1: insertion at the beginning
		...
		else // case 1.2: when list is not empty
			dlla.nodes[dlla.head].prev <- occupiedPos
			dlla.head <- occupiedPos
	else if pos = dlla.size+1 then // case 2: wanna add sth after the last node
...
6) a list could replace any other container, right? list instead of a set - one extra search before ... , list instead of a map
...
- classical answer: use TWO stacks
	...
	- push sth equal to the minimum? push the minimum again -> have a counter maybe.
