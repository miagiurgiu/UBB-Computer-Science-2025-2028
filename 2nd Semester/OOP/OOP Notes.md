...
subject - class, a list of smart pointer to observers, can't have a vector of observers, attach and detach functions - add/remove observer from/to list, notify - noify all observers when sth changes in the data - get though the list of observers => update
...
2.window which allows to see all parcels + add parcel functionality (delivered status initially=0), what is delivered is shown with green
3.deliver button in each window of the deliverer, once pressed it dissapears from the list (delivered status gets set to 1)
4.combo box
5.painter
...
- when you update info make sure other info that depends on that info is also updated even if it does not say so (ex: programmer reviews a file -> getRevised + 1,not just status changed)
...
- when you modify (update) stuff inside repo, do "auto&", not "const auto&"!!!
