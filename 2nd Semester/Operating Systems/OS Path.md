...
int main(int argc, char **argv) {
    ...
    pthread_barrier_init(&barrier, NULL, max - 1); // pointer to barrier, how many threads at barrier
...
- implement this problem with 2 barriers, without data corruption - you should have 2 barriers, as many mutexes as are items are in the array, bc each thread using (i,j) could block strictly those ones, which means that others... - performance comparison
...
void* worker(void* arg) {
	...
	for(int j=0; j<data->cols;j++){ // add all element from the same row
		data->sum +=data->row[j];
	}
	return NULL;
}
...
int main(int argc, char **argv){
	...
	int rows,cols;
	fscanf(f, "%d %d", &rows, &cols); // read matrix dimensions
	int **matrix=malloc(rows*sizeof(int*)); // allocate matrix
...
void* worker(void* arg) {
	ThreadData *data=(ThreadData*)arg; // convert generic pointer back to ThreadData*
	int x=data->number;
	if(x==0){ // while does not take 0 into account
		pthread_mutex_lock(&mutex);
		freq[0]++;
		pthread_mutex_unlock(&mutex);
	}
...
int main(int argc, char **argv){
	...
	for(int i=0;i<m;i++){ // initialise mutexes
		pthread_mutex_init(&checkpoint_mutexes[i],NULL);
	}
	
	pthread_barrier_init(&start_barrier,NULL,n); // barrier waits for all n threads
	...
	for(int i=0;i<n;i++){ // wait for all threads to finish the race
		pthread_join(threads[i],NULL);
	}
...
![[Pasted image 20260623125219.png]]
