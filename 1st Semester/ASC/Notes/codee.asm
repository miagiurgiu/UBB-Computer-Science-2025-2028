...
segment data use32 class=data
    ...
    file_name_input db "input.txt",0 ; inside it we have 10 3 -2 7 -5 4 -8 20 0 but 0 is not part of the array
    file_name_output times 100 db 0
    ; file_name_output db 100 dup(0)
    ; defining the modes - read, write
    mode_r db "r",0
    mode_w db "w", 0
    
    ; defining the descriptors - fin, fout
    ...
    format_int db "%d",0 ; read one integer
    format_pair db "%d %d",0 ; read two integers
    format_out db "%d",10,0 ; print integer + newline
    format_string db "%s", 0

    ; defining the variables for user-entered pairs:
    a dd 0
    b dd 0
